...
class AlterRepr(Representation):
    """ Sequence represented by numbers in even and odd positions."""
    # like [1, 2, 3, 4, 5] --> ([1, 3, 5], [2, 4])

    @classmethod
    def convert(cls, seq):
        """ Convert the sequence to this representation"""
        size = len(seq)
...
class AbsRepr(Representation):
    ...
    def convert(cls, seq):
        ...
        signs = NormalRepr([sgn(i) for i in seq])
        values = NormalRepr([abs(i) for i in seq])
